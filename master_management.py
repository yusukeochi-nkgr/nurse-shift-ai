...
# ==========================================
    ...
    if not org_df.empty and len(ws_dict) > 0:
        ...
        with st.expander("💡 AI最適定数（黄金比）の算出ロジック", expanded=False):
            st.markdown("""
            このシミュレータは、現在のスタッフに加えて「各ランクのダミー要員（幽霊）」を配置し、以下のルールで『最も無駄のないパズル』を解いています。
            
            1. **【現場制約の完全マッピング】** 画面3で設定された「平日/休日別の日勤最低人数・リーダー数・夜勤リーダー数・スコア合計」を1日ずつカレンダー通りに厳密適用します。
            2. **【夜勤上限のハード制約化】** 各個人の夜勤上限超過を「絶対禁止」に設定します。これにより特定のベテランを酷使する不正な計算を完全封鎖します。
            3. **【不足・余剰の同時あぶり出し】** リアルスタッフだけでは回せない日はダミー要員（不足）を投入し、リアルスタッフが過剰な日は丸1ヶ月ベンチ（余剰）へ下げ、必要最小限の適正人員を逆算します。
            """)
        ...
        if st.button("🚀 ランク別 最適定数とギャップを算出", type="primary", use_container_width=True):
            ...
            for j in days:
                ...
                if d_scr > 0:
                    prob += pulp.lpSum([x[i][j]["日"] * score_map.get(w_staff[w_staff["氏名"]==idx_r]["スキルランク"].values[0], 0) if i in I_real else x[i][j]["日"] * score_map.get(rnk_d, 0) for i, rnk_d in dummy_list if i == i]) >= d_scr
